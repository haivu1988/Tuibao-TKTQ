...
export const EmployeeShiftRegistration: React.FC<EmployeeShiftRegistrationProps> = ({
  ...
  const [activeMobileDay, setActiveMobileDay] = useState<DayOfWeekKey>('T2');
  const [registrationDisplayMode, setRegistrationDisplayMode] = useState<'DAY_CARD' | 'FULL_GRID'>('DAY_CARD');
    ...
    <div className="space-y-6">
      ...
      {scheduleConfig.isRegistrationOpen ? (
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-xs space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Bấm vào các ca bạn có thể làm (Ca 1: 8h-13h, Ca 2: 13h-18h, Ca 3: 18h-23h)
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl text-[11px] font-bold">
                <button
                  type="button"
                  onClick={() => setRegistrationDisplayMode('DAY_CARD')}
                  className={`px-2.5 py-1 rounded-lg transition cursor-pointer ${
                    registrationDisplayMode === 'DAY_CARD'
                ...
                <button
                  type="button"
                  onClick={() => setRegistrationDisplayMode('FULL_GRID')}
                  className={`px-2.5 py-1 rounded-lg transition cursor-pointer ${
                    registrationDisplayMode === 'FULL_GRID'
                      ? 'bg-white text-emerald-800 shadow-xs'
                      : 'text-slate-500'
                  }`}
                >
              ...
              <span className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-xl">
                Đã chọn: <strong>{selectedSlots.length}</strong> ca ({selectedSlots.length * 5}h)
              </span>
              ...
              <button
                ...
                onClick={handleSave}
                disabled={isSaving}
                className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 disabled:opacity-50 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-md shadow-emerald-600/20 cursor-pointer"
              >
          ...
          {/* MOBILE DAY-BY-DAY CARD MODE (OPTIMAL FOR SMARTPHONES) */}
          {registrationDisplayMode === 'DAY_CARD' ? (
            <div className="space-y-4">
              {/* Day Carousel Tabs */}
              <div className="flex overflow-x-auto gap-1.5 pb-1 no-scrollbar">
                {DAYS_OF_WEEK.map((d) => {
                  const isActive = activeMobileDay === d.key;
                  const daySlotKeys = shifts.map((s) => getSlotKey(d.key, s.id));
                  const selectedInDayCount = daySlotKeys.filter((k) => selectedSlots.includes(k)).length;
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 min-w-[48px] py-2 px-1.5 rounded-2xl flex flex-col items-center justify-center transition border cursor-pointer ${
                        isActive
                          ? 'bg-emerald-600 text-white border-emerald-600 font-bold shadow-sm shadow-emerald-600/20'
                          : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
                      }`}
                    >
                      <span className="text-xs">{d.label.replace('Thứ ', 'T')}</span>
                      <span className={`text-[9px] mt-0.5 ${isActive ? 'text-emerald-100' : 'text-slate-400'}`}>
                        {d.key === 'T2' ? '24/8' : d.key === 'T3' ? '25/8' : d.key === 'T4' ? '26/8' : d.key === 'T5' ? '27/8' : d.key === 'T6' ? '28/8' : d.key === 'T7' ? '29/8' : '30/8'}
                      </span>
                      {selectedInDayCount > 0 && (
                        <span className={`w-1.5 h-1.5 rounded-full mt-1 ${isActive ? 'bg-white' : 'bg-emerald-500'}`} />
                      )}
                    </button>
                  );
                })}
              ...
              <div className="space-y-2.5">
                <div className="flex items-center justify-between text-xs text-slate-500">
                  <span className="font-bold text-slate-800">
                    {DAYS_OF_WEEK.find((d) => d.key === activeMobileDay)?.label} - Chọn ca rảnh:
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                      const allForDay = shifts.map((s) => getSlotKey(activeMobileDay, s.id));
                      const allSelected = allForDay.every((k) => selectedSlots.includes(k));
                      if (allSelected) {
                        setSelectedSlots((prev) => prev.filter((k) => !allForDay.includes(k)));
                      } else {
                        setSelectedSlots((prev) => Array.from(new Set([...prev, ...allForDay])));
                      }
                    }}
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {shifts.map((shift) => {
                    const slotKey = getSlotKey(activeMobileDay, shift.id);
                    const isSelected = selectedSlots.includes(slotKey);

                    let shiftTitle = 'Ca 1 (Sáng)';
                    let shiftBadge = '08:00 - 13:00 (5 tiếng)';
                    if (shift.id === 'CA_2') {
                      shiftTitle = 'Ca 2 (Chiều)';
                      shiftBadge = '13:00 - 18:00 (5 tiếng)';
                    } else if (shift.id === 'CA_3') {
                      shiftTitle = 'Ca 3 (Tối)';
                      shiftBadge = '18:00 - 23:00 (5 tiếng)';
                    }
                    ...
                    return (
                      <div
                        ...
                        className={`p-3.5 rounded-2xl border transition flex items-center justify-between cursor-pointer ${
                          isSelected
                            ? 'bg-emerald-50 border-emerald-400 text-emerald-950 shadow-xs ring-1 ring-emerald-400/50'
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          >
                            {shift.id === 'CA_1' ? '1' : shift.id === 'CA_2' ? '2' : '3'}
                          </div>
                          <div>
                            <h5 className="font-bold text-xs">{shiftTitle}</h5>
                            <p className="text-[10px] text-slate-500 font-mono mt-0.5">{shiftBadge}</p>
                          </div>
                        </div>
